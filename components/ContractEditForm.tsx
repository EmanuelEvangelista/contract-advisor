...
const ContractEditForm = () => {
    ...
    const fetchContractData = async () => {
      try {
        ...
        if (contractData.expiryDate)
          contractData.expiryDate = new Date(contractData.expiryDate)
            .toISOString()
            .split("T")[0] as any;

        setFields(contractData as any);
      } catch (error) {
        console.log(error);
      } finally {
        setLoading(false);
    ...
    const val =
      type === "checkbox"
        ...
        : type === "number"
          ? value === ""
            ? 0
            : Number(value) // Evita NaN si borran el número
          : value;

    setFields((prev) => {
      const keys = name.split(".");

      // Caso 1: Campo simple (ej: "contractName", "notes")
      if (keys.length === 1) {
        return { ...prev, [name]: val };
      }

      // Caso 2: Campo anidado (ej: "paymentDetails.quintalsPerHa")
      const topLevelKey = keys[0] as keyof ContractFormType;
      const nestedKey = keys[1];

      return {
        ...prev,
        [topLevelKey]: {
          // Forzamos a TS a entender que esto es un objeto para el spread
          ...(prev[topLevelKey] as object),
          [nestedKey]: val,
        },
      };
    });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Creamos una copia para no enviar los objetos File en el JSON
      const dataToSend = { ...fields };
      // Si el backend no procesa archivos por JSON, mejor quitarlos
      delete dataToSend.pdfs;

      const res = await fetch(`/api/contracts/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(dataToSend), // Enviamos la copia limpia
      });
      ...
      >
        ...
        <div className="flex items-center justify-end gap-4 pt-6 border-t">
          ...
          <button
            type="submit"
            disabled={loading}
            className={`px-10 py-3 bg-indigo-600 text-white rounded-xl font-black text-sm uppercase tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all ${
